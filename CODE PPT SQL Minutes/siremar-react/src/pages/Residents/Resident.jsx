...
function Resident() {
    ...
    let url = "http://127.0.0.1:8001/residents";
    useEffect(() => {
        getCounties();
        getResidentById();
    }, []);
    ...
    const handelSubmit = (e) => {
        e.preventDefault();
        if (id) {
            url = "http://127.0.0.1:8001/updateResident"
            Object.assign(formData, { "table_name": tableName });
            let set_values = "phone_number='" + formData["phone_number"] + "',first_name='" + formData["first_name"] + "',last_name='" + formData.last_name + "',dob='" + formData.dob + "',gender='" + formData.gender + "',street1='" + formData.street1 + "',street2='" + formData.street2 + "',email='" + formData.email + "',county_id='" + formData.county_id + "',password='" + formData["password"]  + "'";
            ...
            let set_values = "'" + formData["phone_number"] + "','" + formData["first_name"] + "','" + formData["last_name"] + "','" + formData["dob"] + "','" + formData["gender"] + "','" + formData["street1"] + "','" + (formData["street2"] ? formData["street2"] : " ") + "','" + formData["email"] + "','" + formData["county_id"] + "','" + '2' + "','" + formData["last_name"] + (formData["phone_number"]).substring(1, 4) + "'"
            Object.assign(formData, { "set_values": set_values });
        }
        ...
        axios.post(url, formData).then(res => {
            ...
            if (res.status === 201) {
                ...
                emailjs.send(
                    'service_m5m395k', "template_qgaona9",
                    { message_html: "welcome", from_name: "M", to: "kspavankrishna@gmail.com" },
                    "F6SSSCyg2eiw9FOqi"
                ).then(res => {
