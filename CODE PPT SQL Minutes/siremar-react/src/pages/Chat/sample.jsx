...
function Chat() {
    ...
    const getChats = (id) => {
        ...
        axios.get("http://127.0.0.1:8001/chats/" + room).then(res => {
    ...
    const assignChatRoom = (value) => {
        chatWith = value;
        room = value + "-" + String(loggedUser.id)
    }


    useEffect(() => {
        if (chatWith == "admin") {
            room = "admin-" + String(loggedUser.id)
        } else if (chatWith == "inspector") {
            room = "inspector-" + String(loggedUser.id)
        }
        getChats(room);
        socket.emit("join_room", room);
        socket.on("receive_message", (data) => {
            ...
            axios.post("http://127.0.0.1:8001/chats", { "room": room, "response": false, "message": data.message,user_id:loggedUser.id }).then(res => {
            })
            setMessageReceived((messageList) => [
                ...messageList,
                mess
                    ...
                    <div class="containers">
                        ...
                        <div class="chat-dropdown">
                            ...
                            <div class="chat-dropdown-content">
                                <a onClick={assignChatRoom("admin")}>Admin</a>
