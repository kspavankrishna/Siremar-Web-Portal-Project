...
function School() {
    ...
    let url = "http://127.0.0.1:8001/schools";
    useEffect(() => {
        getCounties();
        getResidentById();
    }, []);
    ...
    const getResidentById = () => {
        ...
        if (id) {
            axios.get("http://127.0.0.1:8001/schools/" + id, {
    ...
    const handelSubmit = (e) => {
        ...
        if (id) {
            formData.id = id;
            url = "http://127.0.0.1:8001/updateSchool"
            Object.assign(formData, { "table_name": tableName });
            let set_values = "phone_number='" + formData["phone_number"] + "'," + "name='" + formData["name"] + "',street1='" + formData.street1 + "',street2='" + formData.street2 + "',county_id='" + formData.county_id + "'";
            Object.assign(formData, { "set_values": set_values });
            axios.post(url, formData).then(res => {

                if (res.status === 201||res.status === 200) {

                    navigate("/schools?role=" + role);
                } else {
                    alert('Failed');
                }
            })
        } else {
            Object.assign(formData, { "table_name": tableName + "(name,phone_number,street1,street2,county_id)" });
            let set_values = "'" + formData["name"] + "','" + formData.phone_number + "','" + formData["street1"] + "','" + (formData["street2"] ? formData["street2"] : " ") + "','" + formData["county_id"] + "'";
            Object.assign(formData, { "set_values": set_values });
                            ...
                            <Form class="register-form" action="" method="">
                                <div class="flex-row">
                                    <Form.Label class="custom-form-label-wrapper">
                                        ...
                                        <Form.Control name="name" value={formData["name"]} onChange={handleChange} type="text" placeholder="Enter school name" required />
                                    </Form.Label>
