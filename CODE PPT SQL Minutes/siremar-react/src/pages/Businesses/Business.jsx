...
function Business() {
    ...
    let url = "http://127.0.0.1:8001/businesses";
    useEffect(() => {
        getCounties();
        getResidentById();
    }, []);
    ...
    const getResidentById = () => {
        ...
        if (id) {
            axios.get("http://127.0.0.1:8001/businesses/" + id)
                .then(res => {
    ...
    const handelSubmit = (e) => {
        ...
        if (id) {
            formData.id=id;
            url = "http://127.0.0.1:8001/updateBusiness"
            Object.assign(formData, { "table_name": tableName });
            let set_values = "description='" + formData["description"] + "'," + "type='" + formData["type"] + "',street1='" + formData.street1 + "',street2='" + formData.street2 + "',phone_number='" + formData["phone_number"] + "',county_id='" + formData.county_id + "',benefits='" + formData.benefits+"'";
            Object.assign(formData, { "set_values": set_values });
        } else {
            Object.assign(formData, { "table_name": tableName + "(description,type,street1,street2,phone_number,county_id,benefits)" });
            let set_values = "'" + formData["description"] + "','" + formData.type + "','" + formData["street1"] + "','" + (formData["street2"] ? formData["street2"] : " ") + "','" +formData.phone_number+"','"+ formData["county_id"] + "','"+formData.benefits+"'";
            Object.assign(formData, { "set_values": set_values });
        }
                            ...
                            <Form class="register-form" action="" method="">
                                <div class="flex-row">
                                    <Form.Label class="custom-form-label-wrapper">
                                        <p class="custom-form-label">Description</p>
                                        <Form.Control name="description" value={formData["description"]} onChange={handleChange} type="text" placeholder="Enter business description" required />
                                    </Form.Label>

                                    <Form.Label class="custom-form-label-wrapper">
                                        <p class="custom-form-label">Type</p>
                                        <Form.Control name="type" value={formData["type"]} onChange={handleChange} type="text" placeholder="Enter business type" required />
                                    </Form.Label>
                                    ...
                                    <Form.Label class="custom-form-label-wrapper">
                                        ...
                                        <Form.Control name="benefits" value={formData["benefits"]} onChange={handleChange} type="text" placeholder="Enter the benefits who register for the business" required />
                                    </Form.Label>
                                </div>
