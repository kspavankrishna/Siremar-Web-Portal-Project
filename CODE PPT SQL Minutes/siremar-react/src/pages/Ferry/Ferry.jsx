...
function Ferry() {
    ...
    let url = "http://127.0.0.1:8001/ferry";
    useEffect(() => {
        getCounties();
        getResidentById();
    }, []);
    ...
    const getResidentById = () => {
        ...
        if (id) {
            axios.get("http://127.0.0.1:8001/ferry/" + id)
                .then(res => {
    ...
    const handelSubmit = (e) => {
        ...
        if (id) {
            formData.id=id;
            url = "http://127.0.0.1:8001/updateFerry"
            Object.assign(formData, { "table_name": tableName });
            let set_values = "date_of_ferry='" + formData["date_of_ferry"] + "'," + "name='" + formData["name"] +"',ferry_time='" + formData["ferry_time"]+"',street1='" + formData.street1 + "',street2='" + formData.street2 + "',county_id='" + formData.county_id + "',benefits='" + formData.benefits+"'";
            Object.assign(formData, { "set_values": set_values });
        } else {
            Object.assign(formData, { "table_name": tableName + "(name,date_of_ferry,ferry_time,street1,street2,county_id,benefits)" });
            let set_values = "'" + formData["name"] + "','" + formData.date_of_ferry+ "','" + formData["ferry_time"]  + "','" + formData["street1"] + "','" + (formData["street2"] ? formData["street2"] : " ") + "','" + formData["county_id"] + "','" + formData.benefits + "'";
            Object.assign(formData, { "set_values": set_values });
        }
                            ...
                            <Form class="register-form" action="" method="">
                                <div class="flex-row">
                                    <Form.Label class="custom-form-label-wrapper">
                                        ...
                                        <Form.Control name="name" value={formData["name"]} onChange={handleChange} type="text" placeholder="Enter ferry name" required />
                                    </Form.Label>


                                    <Form.Label class="custom-form-label-wrapper">
                                        <p class="custom-form-label">Date</p>
                                        <Form.Control name="date_of_ferry" value={formData["date_of_ferry"]} onChange={handleChange} type="date" placeholder="Enter date" required />
                                    </Form.Label>
                                    <Form.Label class="custom-form-label-wrapper">
                                        <p class="custom-form-label">Time</p>
                                        <Form.Control name="ferry_time" value={formData["ferry_time"]} onChange={handleChange} type="time" placeholder="Enter time" />
                                    </Form.Label>
