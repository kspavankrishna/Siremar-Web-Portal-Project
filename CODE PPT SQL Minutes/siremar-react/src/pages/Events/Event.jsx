...
function Event() {
    ...
    let url = "http://127.0.0.1:8001/events";
    useEffect(() => {
        getCounties();
        getResidentById();
    }, []);
    ...
    const getResidentById = () => {
        ...
        if (id) {
            axios.get("http://127.0.0.1:8001/events/" + id)
                .then(res => {
    ...
    const handelSubmit = (e) => {
        ...
        if (id) {
            formData.id=id;
            url = "http://127.0.0.1:8001/updateEvent"
            Object.assign(formData, { "table_name": tableName });
            let set_values = "date_of_event='" + formData["date_of_event"] + "'," + "name='" + formData["name"] + "',phone_number='" + formData["phone_number"] + "',event_time='" + formData["event_time"] + "',street1='" + formData.street1 + "',street2='" + formData.street2 + "',county_id='" + formData.county_id + "',benefits='" + formData.benefits + "'";
            Object.assign(formData, { "set_values": set_values });
        } else {
            Object.assign(formData, { "table_name": tableName + "(name,phone_number,date_of_event,event_time,street1,street2,county_id,benefits)" });
            let set_values = "'" + formData["name"] + "','" +  formData["phone_number"] + "','" +formData.date_of_event + "','" + formData["event_time"] + "','" + formData["street1"] + "','" + (formData["street2"] ? formData["street2"] : " ") + "','" + formData["county_id"] + "','" + formData.benefits + "'";
            Object.assign(formData, { "set_values": set_values });
        }
                            ...
                            <Form class="register-form" action="" method="">
                                <div class="flex-row">
                                    <Form.Label class="custom-form-label-wrapper">
                                        ...
                                        <Form.Control name="name" value={formData["name"]} onChange={handleChange} type="text" placeholder="Enter event name" required />
                                    </Form.Label>


                                    <Form.Label class="custom-form-label-wrapper">
                                        <p class="custom-form-label">Date</p>
                                        <Form.Control name="date_of_event" value={formData["date_of_event"]} onChange={handleChange} type="date" placeholder="Enter date" required />
                                    </Form.Label>
                                    <Form.Label class="custom-form-label-wrapper">
                                        <p class="custom-form-label">Time</p>
                                        <Form.Control name="event_time" value={formData["event_time"]} onChange={handleChange} type="time" placeholder="Enter time" />
                                    </Form.Label>
